...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if type(procedure) is MacroProcedure:
            macro = procedure.apply_macro(rest, env)
            return scheme_eval(macro, env)
        #print(expr)
        check_procedure(procedure)
        rest = rest.map(lambda arg: scheme_eval(arg, env))
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("too much or too few vals given")
        if formals == nil:
            return child
...
def do_quote_form(expressions, env):
    ...
    return expressions.first + do_quote_form(expressions.second, env)
    # END PROBLEM 7
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, args, env):
        while self.formals != nil:
            env.define(self.formals.first, args.first)
            self.formals = self.formals.second
            args = args.second
        return env
    # END PROBLEM 16
